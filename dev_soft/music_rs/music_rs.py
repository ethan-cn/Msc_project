...
def get_rid2name(file_name):
    """
    Get name to ID and ID to name mappings, including songs or users, and so on
    :param file_name: The path name of the file to be parsed, and the ID of the first column and the name of the second
                        column of each row in the file are separated by tabs.
    :return: The two dictionaries
    rid_to_name:The key is rid, the value is name, and there is only one key-value pair
    name_to_rid = {} key is name, value is rid, and there is only one key-value pair
    """
    rid_to_name = {}
    name_to_rid = {}
    # Open the file and specify the encoding format (here the specified format required for this data)
    with io.open(file_name, 'r', encoding='utf-8') as f:
        # next(f)  # Skip the column names in the first row and read from the second row
        # Read one line at a time, and the elements in each line are separated by the TAB character "\t"
        for line in f:
            # Dump each element into the array
            line = line.split('\t')
            # The first element is an ID and the second element is a name
            rid_to_name[line[0]] = line[1]
            name_to_rid[line[1]] = line[0]
    return rid_to_name, name_to_rid


# Gets the mapping of song name to song ID and song ID to song name
item_rid_to_name, item_name_to_rid = get_rid2name('dataset/min_song_info.txt')

# Get user name to ID and user ID to name mappings
...
def get_trainset_algo():
    """
    The algorithm is trained using the training set
